...
function serviceHostnames(config) {
    return Object.fromEntries(Object.entries(config.services).map(([name, cfg]) => [name, cfg.hostname]));
}
function renderServiceEnv(slug, tld, worktreeName, services, ports) {
    const env = {
        WTENV_SLUG: slug,
...
function updateEnvFile(path, generated) {
...
export async function renameSlug(slug, options = {}) {
    validateSlug(slug);
    ...
    try {
        ...
        if (bareLocals.length > 0)
            registerMdnsHosts(slug, bareLocals);
    ...
    updateEnvFile(join(cwd, options.envFile ?? ".env.worktree"), renderServiceEnv(slug, config.tld, wt.name, config.services, ports));
