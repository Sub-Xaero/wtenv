...
export async function setup(): Promise<void> {
  ...
  if (existingResolver.trim() !== resolverContent.trim()) {
    ...
    const result = spawnSync(
      "sudo",
      ["bash", "-c", `mkdir -p /etc/resolver && printf '${resolverContent}' > ${RESOLVER_PATH}`],
      { stdio: "inherit" }
    );
    if (result.status !== 0) {
      console.log(
  ...
  if (existingPfRule.trim() !== pfRule.trim()) {
    ...
    const pfResult = spawnSync(
      "sudo",
      ["bash", "-c", `printf '${pfRule}' > ${PF_ANCHOR_PATH} && pfctl -ef ${PF_ANCHOR_PATH} 2>/dev/null; true`],
      { stdio: "inherit" }
    );
