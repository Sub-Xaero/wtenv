  ...
  if (worktrees.length === 0) {
    ...
    for (const wt of worktrees) {
      ...
      check(
        ...
        `run 'wtenv deregister --city ${wt.city}' or 'wtenv deregister --stale'`
      );
      ...
      check(
        ...
        `re-register with 'wtenv reregister' or cleanup with 'wtenv deregister --city ${wt.city}'`
      );
      ...
      for (const [service, port] of Object.entries(wt.ports)) {
        ...
        if (lsof.status === 0 && lsof.stdout.trim()) {
          check(
            ...
            "warn",
            "something already listening — may conflict on start"
          );
        }
      }
    }
