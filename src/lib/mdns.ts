...
function label(projectName: string): string {
...
export function registerMdnsHosts(projectName: string, hostnames: string[]): void {
  ...
  const dnsSdCmds = hostnames
    .map(
      (h, i) =>
        `/usr/bin/dns-sd -P "wtenv-${projectName}-${i}" _wtenv._tcp local. 1 "${h}" 127.0.0.1 &`
    )
    .join("\n");
...
export function deregisterMdnsHosts(projectName: string): void {
  const plist = plistPath(projectName);
  if (existsSync(plist)) {
    spawnSync("launchctl", ["unload", plist], { stdio: "ignore" });
    unlinkSync(plist);
