...
export function sudoState(): SudoState {
  if (spawnSync("which", ["sudo"], { stdio: "ignore" }).status !== 0) return "no-sudo";
  if (spawnSync("sudo", ["-n", "-v"], { stdio: "ignore" }).status === 0) return "cached";
...
export function canSudoSilently(): boolean {
  const s = sudoState();
...
export function requireSudoOrSkip(reason: string): boolean {
  if (canSudoSilently()) return true;
...
export function sudoExec(argv: string[], opts: { stdio?: "inherit" | "ignore" | "pipe" } = {}): boolean {
